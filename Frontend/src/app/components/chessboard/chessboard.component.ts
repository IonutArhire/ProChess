import { Input, Output, EventEmitter, ViewChild, ElementRef, Component, Renderer2, OnDestroy, AfterViewInit, OnInit, NgZone, HostListener, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
export class ChessboardComponent implements OnDestroy, AfterViewInit, OnInit
{
	...
	@ViewChild("pawnPromotion") pawnPromotion: PawnPromotionComponent;
	...
	getPieceImage(pieceName)
	{
		return ConfigService.themePath + pieceName + ".png";
	}
	...
	@Output("promotedPiece")
	promotedPiece = new EventEmitter<string>();

	// Internals
	board : ChessSquareComponent[][] = [];
	boardCreated = false;
	...
	createBoard()
	{
		...
		if(this.isFlipped)
		{
			for(var x=this.boardHeight-1;x>=0;--x)
			{
				var isLight = (x % 2) == 0;
				var row = [];
				for(var y=this.boardWidth-1;y>=0;--y)
				{
		...
		{
			let square = this.getSquareXY((this.isFlipped?0:this.boardHeight-1), i);
			square.addIndicator("horizontal", CHESS_SIDES[i]);
		}

		for(var i=0;i<this.boardHeight;++i)
		{
			let square = this.getSquareXY(i, (this.isFlipped?this.boardWidth-1:0));
	...
	getSquareByPosition(pos : Position) : ChessSquareComponent
	{
		return this.getSquareXY(pos.x, pos.y);
	}

	getSquareXY(x, y): ChessSquareComponent
	{
		if(x < this.boardHeight && x >= 0 && y < this.boardWidth && y >= 0)
		{
	...
	getBoard()
	{
		...
		this.forEachSquare((square, x, y) => {
			...
			if(square.piece)
			{
				boardData[fen] = [square.piece.color, square.piece.pieceType];
			}
		});
		return boardData;
	}
	...
	getFEN(square: ChessSquareComponent)
	{
		...
		return CHESS_SIDES[square.y] + ( this.boardHeight - square.x);
	}

	getSquare(fen:string)
	{
		var x = this.boardHeight - parseInt(fen[1]);
		var y = CHESS_SIDES.indexOf(fen[0]);
	...
	promote(movingPlayerColor: string): void
	{
	...
	promotionChosen(piece: string): void
	{
		this.promotedPiece.emit(piece);
		this.isPawnPromotion = false;
		this.cdRef.detectChanges();
	}
	
	swapPieces(square1: ChessSquareComponent, square2: ChessSquareComponent) {
		var aux = square1.piece;
		square1.setPieceByModel(square2.piece);
		square2.setPieceByModel(aux);
	}
}
